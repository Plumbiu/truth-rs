...
function handleGraphRoot() {
  if (legend !== 'Graph' || pkgName.value === '__root__') return
  if (!isAim.value) target = pkgName.value
...
<template>
  <div class="header">
    <a
      href="https://github.com/truthRestorer/truth-cli"
      target="_blank"
      class="left"
    >
      TRUTH-RS
    ...
    <div class="right">
      ...
      <ElButton @click="() => collapseNode(legend)">收缩节点</ElButton>
      <ElButton @click="drawer = !drawer">
        {{ drawer ? '关闭' : '打开' }}信息框
      </ElButton>
      <ElButton @click="handleLegend">切换图表</ElButton>
      <Link />
    </div>
  </div>
</template>
